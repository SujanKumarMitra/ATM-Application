...
public class Balance_Withdraw {
	...
	public static void execute() {
		Balance_Withdraw.main(null);
	}
	private static void main(String[] args) {
		JFrame f = new JFrame("Balance Withdraw");
				...
				try {
					...
					while (rs.next()) {
						double avail_balance = rs.getDouble(1);
							...
							String withdraw= JOptionPane.showInputDialog(f, "Enter amount to withdraw:");
							withdraw_amount = Double.parseDouble(withdraw);
						while (avail_balance - withdraw_amount<= 5000) {
							withdraw= JOptionPane.showInputDialog(f, "Remaining balance getting below minimum! Enter valid amount:");
							withdraw_amount = Double.parseDouble(withdraw);
						}
